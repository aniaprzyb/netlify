---
number: "335"
date: "2025-12-01T18:00:00+02:00"
title: "Transformacja G2A na wielozespołowy Model Produktowy"
speakers: "Krzysztof Niewiński"
lang: "pl"
sponsors: ["DXC", "QAgile"]
videoURL: "https://youtu.be/TvynRyiZy2M?si=076lMXDupMAUEFrk"
slidesURL: ""
draft: false
---

## Prelekcja

Do kwietnia 2025 w G2A mieliśmy 12 zespołów, 12 „Product Ownerów”, 12 backlogów – i wiele powodów, dla których **Time to Market był bardzo długi**, a dostarczanie realnej wartości trudne.

Wielkie plany, niekończąca się koordynacja, każdy zespół zajęty… a wartość produktu wcale nie rosła.

> „Zdaliśmy sobie sprawę, że bez zmiany systemu pracy nie uda nam się zboostować rezultatów”  
> – jak powiedzieli liderzy działu produktu.

W styczniu 2025 zaczęliśmy przygotowania.  
W kwietniu przeprowadziliśmy **Warsztat Samoformujących się Zespołów**, projektując nowy system pracy.

Od tego momentu eksperymentujemy z praktykami i optymalizujemy dostarczanie maksymalnej wartości.

**Efekt?**
- Time to Market skrócony **5-krotnie**
- Realizacja prac z wierzchołka **globalnego Product Backlogu**
- 12 zespołów skupionych na **wspólnych Outcome’ach**, zamiast lokalnej optymalizacji

To historia prawdziwej transformacji wielozespołowej organizacji produktowej.  
Bez upiększania. Z błędami, z których się uczyliśmy. Z rozwiązaniami, które działają.

### W tej prezentacji zobaczysz:

- Punkt wyjścia – wiele zespołów Scrumowych skupionych na outputcie, lokalnie optymalizujących, bez wpływu na globalny impact (spoiler: jak wszędzie)
- Dlaczego „product per zespół" to pułapka – jak lokalna optymalizacja sabotuje biznes
 -Jak przekonać ludzi do zmiany – jeden wspólny produkt to nie chaos, ale klarowność i realny impact biznesowy
- Co się zmienia, gdy przestajesz pytać „czy zespół jest zajęty", a zaczynasz pytać „czy dostarczamy maksymalną globalną wartość"
- Konkretne lekcje – błędy, których nie musisz popełniać, i praktyki gotowe do wdrożenia już jutro
- To historia prawdziwej transformacji wielozespołowej organizacji produktowej. Bez upiększania. Z błędami, z których się uczyliśmy. Z rozwiązaniami, które działają.
- Jeśli zastanawiasz się, jak przejść od wielu zespołów pracujących „efektywnie" do organizacji dostarczającej wyniki – ta prelekcja jest dla Ciebie.

***
## Poznaj prowadzącego

Cześć, jestem **Krzysztof Niewiński** – Transformation Manager & Product Coach.

Od ponad 20 lat pracuję w produktowych zespołach technologicznych. Zaczynałem jako programista, później Agile Coach, aż w końcu odkryłem swoją prawdziwą pasję – **łączenie biznesu z technologią i ludźmi** przy tworzeniu produktów.

Dziś pomagam firmom projektować wielozespołowe organizacje produktowe, które naprawdę **dostarczają wyniki**.  
Szkolę liderów, doradzam zespołom i mentoruję produktowców w budowaniu produktów w dużej skali.

Nie uczę pustej teorii – dzielę się praktyką sprawdzoną w realnych zespołach i firmach, od startupów po korporacje.

Więcej o mnie znajdziesz na:  
https://productorg.com/o-mnie, https://www.linkedin.com/in/krzysztofniewinski
